...
def notes():
    return f"\tOccurences | Character(s)\n{('-' * 45)}"
...
def yarascan(filename):
    ...
    try:
        ...
        if len(ymatch):
            print(f"[+] YARA hit(s): {ymatch}")
            print(f"[+] Possible XOR key: {key}")
            print(f"[+] Un-XOR'ed file should be examined: {unxored}")
            print("[+] Stopping processing")
            sys.exit()
    except Exception as msg:
        print(f"[!] YARA compile error: {msg}")
        sys.exit()

# HEX it up?
def hexMeBro(filename):
    print(f"[+] HEXing {('.'*32 + ': ')}{filename}")
    with open(filename, 'rb') as f:
        content = f.read()
        print(f"[+] Saving as {('.'*29 + ': ')}{new_file}")
        with open(new_file, 'wb') as nf:
            nf.write(binascii.hexlify(content))

# Attempt to guess the XOR key with frequency analysis
def guess(filename):
    print(f"[+] Attempting to guess the XOR key of ....: {filename}")
    ...
    """
    results = []
    results.append(f"[+] Size of content.......................: {length}")
    results.append(f"[+] Total pages (1024k)...................: {length / int(1024)}")
    results.append(f"[+] Total contiguous 512 chunks...........: {length / int(512)}")
    results.append(header("[+] Top (5) overall chars"))
    results.append(notes())
    results.append(charCounter())
    results.append("")
    results.append(f"[+] Total number of unique 512 chunks.....: {len(char_mucho)}")
...
if args['convert']:
    ...
    for key,occur in keys:
        unxored = f"{filename}.{str(i)}.unxored"
